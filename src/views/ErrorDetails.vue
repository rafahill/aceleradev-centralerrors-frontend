<template>
     <v-card v-if="error">
        <v-card-title class="headline">{{error.title}} <v-icon right :color="getColor(error.errorCode)" left dark>{{ error.iconLevel }}</v-icon></v-card-title>
  ...
  methods: {
    getError(){
      this.teste = api.findById(this.$route.params.id).then(data => {
          ...
          if (this.error.errorCode >= 300 && this.error.errorCode <= 999) this.error.iconLevel = 'update'
